...
const docs = readFileSync(
  new URL("../../app/developer-workspace/docs/page.tsx", import.meta.url),
  "utf8",
);
const feedback = readFileSync(
  new URL(
    "../../app/developer-workspace/beta-feedback/BetaFeedbackForm.tsx",
    import.meta.url,
  ),
  "utf8",
);
const requestContext = readFileSync(
  new URL(
    "../../lib/developer-workspace/workspaceRequestContext.ts",
    import.meta.url,
  ),
  "utf8",
);
const packager = readFileSync(
  new URL("../../scripts/package-developer-workspace.mjs", import.meta.url),
  "utf8",
);
...
const layout = readFileSync(
  new URL("../../app/developer-workspace/layout.tsx", import.meta.url),
  "utf8",
);
const workspace = readFileSync(
  new URL(
    "../../app/tools/developer-workspace/DeveloperWorkspace.tsx",
    import.meta.url,
  ),
  "utf8",
);
const buildDiagnostics = readFileSync(
  new URL("../../lib/developer-workspace/buildDiagnostics.ts", import.meta.url),
  "utf8",
);
const supportDownload = readFileSync(
  new URL(
    "../../app/developer-workspace/SupportReportDownloadButton.tsx",
    import.meta.url,
  ),
  "utf8",
);
...
const windowsRelease = readFileSync(
  new URL(
    "../../.github/workflows/developer-workspace-beta-release.yml",
    import.meta.url,
  ),
  "utf8",
);
...
describe("Developer Workspace coder beta handoff", () => {
  ...
  it("keeps feedback local and exports a privacy-reminded report", () => {
    ...
    expect(feedback).toContain(
      "No source code, credentials, environment variables, or absolute paths",
    );
  ...
  it("packages a loopback-only Windows runtime that opens Chrome", () => {
    expect(requestContext).toContain(
      'DEVELOPER_WORKSPACE_LOCAL_RUNTIME === "1"',
    );
  ...
  it("downloads support reports explicitly and can use runtime-bundled TypeScript", () => {
    ...
    expect(buildDiagnostics).toContain(
  ...
  it("publishes the Windows beta only after packaged smoke tests pass", () => {
    ...
    expect(windowsRelease).toContain("$health.report.ready");
