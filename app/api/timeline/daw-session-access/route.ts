...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ access: access.decision, capabilities: access.decision.role === "owner"
      ? ["session:read", "workflow:read", "feedback:create", "feedback:respond", "transport:read"]
      : ["session:read", "workflow:read", "feedback:create", "feedback:respond", "transport:read"], receipts: data ?? [] }, { headers: { "Cache-Control": "no-store" } });
