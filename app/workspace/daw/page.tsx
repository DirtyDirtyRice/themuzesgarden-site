...
function SessionHealth({ session, snapshot, busy, onValidate, onInitialize, onActivate, onResume }: { session: DawSnapshot["sessions"][number]; snapshot: DawSnapshot; busy: boolean; onValidate: () => void; onInitialize: () => void; onActivate: () => void; onResume: () => void }) {
  ...
  const repair = timelineDawReadinessRepairAction({ state: session.state, readinessReady: session.readiness.ready });
  const transportRepair = timelineDawTransportInitializationAction({ state: session.state, readinessReady: session.readiness.ready }, snapshot.resumeBySessionId?.[session.id]);
  const activation = timelineDawSessionActivationAction({ state: session.state, readinessReady: session.readiness.ready }, snapshot.resumeBySessionId?.[session.id]);
  const resume = timelineDawSuspendedSessionResumeAction({ state: session.state, readinessReady: session.readiness.ready }, snapshot.resumeBySessionId?.[session.id]);
  return <><p className={`mt-2 text-sm font-semibold ${health.state === "ready" ? "text-emerald-200" : "text-amber-200"}`}>{health.label} · {health.nextAction}</p>{repair?.action === "validate" ? <button type="button" className={`${button} mt-3`} disabled={busy} onClick={onValidate}>{busy ? "Validating…" : repair.label}</button> : null}{transportRepair ? <button type="button" className={`${button} mt-3`} disabled={busy} onClick={onInitialize}>{busy ? "Initializing…" : transportRepair.label}</button> : null}{activation ? <button type="button" className={`${button} mt-3`} disabled={busy} onClick={onActivate}>{busy ? "Activating…" : activation.label}</button> : null}{resume ? <button type="button" className={`${button} mt-3`} disabled={busy} onClick={onResume}>{busy ? "Resuming…" : resume.label}</button> : null}</>;
}
  ...
  return (
    <main className="min-h-screen bg-black px-5 py-8 text-white">
      <section className="mx-auto max-w-6xl space-y-6">
        ...
        {!loading ? studios.map(({ project, snapshot, error: studioError }) => <section key={project.id} className="rounded-3xl border border-white/15 bg-[#080808] p-6"><div className="flex flex-wrap justify-between gap-3"><div><p className="text-xs uppercase text-white/40">{project.kind} · {project.visibility}</p><h2 className="text-2xl font-black">{project.title || "Untitled Project"}</h2><p className="text-xs text-white/40">Workspace revision {snapshot?.workspaceRevision ?? "unavailable"}</p></div><Link href={`/workspace/projects/${encodeURIComponent(project.id)}`} className={button}>Manage Project Studio</Link></div>{studioError ? <p role="alert" className="mt-4 text-amber-200">{studioError}</p> : null}{snapshot?.sessions.length === 0 ? <p className="mt-4 text-white/55">No sessions yet. Open the project to create one.</p> : null}<div className="mt-5 grid gap-3 lg:grid-cols-2">{snapshot?.sessions.filter((session) => session.state !== "closed").map((session) => <article key={session.id} className="rounded-2xl border border-white/10 bg-black p-4"><div className="flex justify-between gap-3"><h3 className="font-black">{session.name}</h3><span className="text-xs font-black uppercase text-emerald-300">{session.state}</span></div><p className="mt-2 text-sm text-white/55">Engines {session.readiness.completed}/{session.readiness.required} · Revision {session.revision}</p><SessionHealth session={session} snapshot={snapshot} busy={busySessionId === session.id} onValidate={() => void validateSession(snapshot, session)} onInitialize={() => void initializeTransport(snapshot, session)} onActivate={() => void activateSession(snapshot, session)} onResume={() => void resumeSession(snapshot, session)} /><Link href={`/workspace/projects/${encodeURIComponent(project.id)}/studio/${encodeURIComponent(session.id)}`} className={`${button} mt-4`}>Enter Workspace</Link></article>)}</div></section>) : null}
      </section>
    </main>
  );
}
