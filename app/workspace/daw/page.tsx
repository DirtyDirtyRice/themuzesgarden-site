  ...
  return (
    <main className="min-h-screen bg-black px-5 py-8 text-white">
      <section className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="rounded-2xl border border-white/10 bg-[#080808] p-4"><label className="text-sm font-black" htmlFor="daw-open-session-search">Search open sessions</label><div className="mt-2 grid gap-2 sm:grid-cols-[1fr_auto]"><input id="daw-open-session-search" type="search" maxLength={100} value={recentQuery} onChange={(event) => setRecentQuery(event.target.value)} placeholder="Session, song, or project" className="w-full rounded-xl border border-white/15 bg-black px-3 py-2"/><select aria-label="Filter open sessions by state" value={recentStateFilter} onChange={(event) => setRecentStateFilter(event.target.value as TimelineDawOpenSessionFilter)} className="rounded-xl border border-white/15 bg-black px-3 py-2"><option value="all">All</option><option value="needs-setup">Needs Setup</option><option value="ready">Ready</option><option value="active">Active</option><option value="suspended">Suspended</option></select></div><p className="mt-2 text-xs text-white/45">Showing {filteredRecent.sessions.length} of {filteredRecent.matchingCount} matches · {filteredRecent.totalOpenCount} total open</p></section>
