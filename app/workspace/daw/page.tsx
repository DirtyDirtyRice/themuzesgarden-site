...
import { TIMELINE_DAW_MUSICIAN_SESSION_STATE } from "../../../lib/timeline/TimelineDawMusicianSessionControls";
    ...
    <main className="min-h-screen bg-black px-5 py-8 text-white">
      <section className="mx-auto max-w-6xl space-y-6">
        ...
        <section id="daw-navigation-help" className="scroll-mt-24 rounded-3xl border border-sky-300/30 bg-sky-300/[0.05] p-6">
          ...
          <p className="mt-2 text-white/65">Choose the thing you want to do. Open the directions and follow one step at a time.</p>
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {TIMELINE_DAW_NAVIGATION_DESTINATIONS.map((destination) => <details id={destination.id === "start-song" ? "daw-start-song-help" : undefined} key={destination.id} className="scroll-mt-24 rounded-2xl border border-white/15 bg-black p-4"><summary className="cursor-pointer text-lg font-black text-white">{destination.label}</summary><p className="mt-2 text-sm text-white/60">{destination.purpose}</p><ol className="mt-3 list-decimal space-y-2 pl-5 text-sm text-white/80">{destination.steps.map((step) => <li key={step}>{step}</li>)}</ol><Link href={destination.href} className={`${button} mt-4`}><span className="text-black">{destination.actionLabel}</span></Link></details>)}
          </div>
        </section>
        <section id="daw-open-sessions" aria-busy={!recentPreferencesLoaded} className="scroll-mt-24 rounded-2xl border border-white/10 bg-[#080808] p-4"><RecentSessionViewControls loaded={recentPreferencesLoaded} query={recentQuery} stateFilter={recentStateFilter} sort={recentSort} result={filteredRecent} searchLabel="Search open sessions" searchId="daw-open-session-search" searchPlaceholder="Session, song, or project" buttonClassName={button} inputClassName="w-full rounded-xl border border-white/15 bg-black px-3 py-2" selectClassName="rounded-xl border border-white/15 bg-black px-3 py-2" statusClassName="mt-2 text-xs text-white/45" loadingClassName="text-sm text-white/45" onQueryChange={setRecentQuery} onStateFilterChange={setRecentStateFilter} onSortChange={setRecentSort} /></section>
        {!loading && filteredRecent.totalOpenCount > 0 && filteredRecent.matchingCount === 0 ? <RecentSessionEmptyState title="No matching open sessions" message="The current search and state filter hide every open session. Your selected sort order will stay unchanged." actionLabel="Clear Search and State Filter" buttonClassName={button} onAction={() => { setRecentQuery(""); setRecentStateFilter("all"); }} /> : null}
        ...
        {!loading && studios.length === 0 ? <RecentSessionEmptyState title="Create your first project" message="A protected DAW session starts from music linked to one of your owner projects." actionLabel="Open Projects" buttonClassName={button} href="/workspace/projects" /> : null}
        {!loading ? studios.map(({ project, snapshot, error: studioError }) => <section key={project.id} className="rounded-3xl border border-white/15 bg-[#080808] p-6"><div className="flex flex-wrap justify-between gap-3"><div><p className="text-xs uppercase text-white/40">{project.kind} · {project.visibility}</p><h2 className="text-2xl font-black">{project.title || "Untitled Project"}</h2><p className="text-xs text-white/40">Workspace revision {snapshot?.workspaceRevision ?? "unavailable"}</p></div><Link href={`/workspace/projects/${encodeURIComponent(project.id)}`} className={button}><span className="text-black">Open Project and Start a Song</span></Link></div>{studioError ? <p role="alert" className="mt-4 text-amber-200">{studioError}</p> : null}{snapshot?.sessions.length === 0 ? <p className="mt-4 text-white/55">No sessions yet. Use Start a Song Here above.</p> : null}<div className="mt-5 grid gap-3 lg:grid-cols-2">{snapshot?.sessions.filter((session) => visibleRecentIds.has(session.id)).map((session) => <article key={session.id} className="rounded-2xl border border-white/10 bg-black p-4"><div className="flex justify-between gap-3"><h3 className="font-black">{session.name}</h3><span className="text-xs font-black uppercase text-emerald-300">{TIMELINE_DAW_MUSICIAN_SESSION_STATE[session.state].label}</span></div><p className="mt-2 text-sm text-white/55">Setup {session.readiness.completed}/{session.readiness.required} ready</p><SessionHealth projectId={project.id} session={session} snapshot={snapshot} busy={busySessionId === session.id} onValidate={() => void validateSession(snapshot, session)} onInitialize={() => void initializeTransport(snapshot, session)} onActivate={() => void activateSession(snapshot, session)} onResume={() => void resumeSession(snapshot, session)} /></article>)}</div>{snapshot ? <ClosedSessionArchive project={project} snapshot={snapshot} /> : null}</section>) : null}
      </section>
    </main>
  );
}
