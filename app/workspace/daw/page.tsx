...
import { changeDawSession, loadDawSnapshot } from "../projects/[id]/projectDawApi";
...
function SessionHealth({ session, snapshot, busy, onValidate }: { session: DawSnapshot["sessions"][number]; snapshot: DawSnapshot; busy: boolean; onValidate: () => void }) {
  ...
  return <><p className={`mt-2 text-sm font-semibold ${health.state === "ready" ? "text-emerald-200" : "text-amber-200"}`}>{health.label} · {health.nextAction}</p>{repair?.action === "validate" ? <button type="button" className={`${button} mt-3`} disabled={busy} onClick={onValidate}>{busy ? "Validating…" : repair.label}</button> : null}</>;
}
