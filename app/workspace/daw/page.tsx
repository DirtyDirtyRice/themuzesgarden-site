...
import { loadDawSnapshot } from "../projects/[id]/projectDawApi";
...
function SessionHealth({ session, snapshot }: { session: DawSnapshot["sessions"][number]; snapshot: DawSnapshot }) {
  ...
  return <p className={`mt-2 text-sm font-semibold ${health.state === "ready" ? "text-emerald-200" : "text-amber-200"}`}>{health.label} · {health.nextAction}</p>;
}
