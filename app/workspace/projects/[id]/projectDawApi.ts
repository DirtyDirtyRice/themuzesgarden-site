...
export function assignDawPrivateFolderBus(sessionId: string, laneIds: string[], busId: string | null): Promise<{ lanes: Array<{ laneId: string; busId: string | null; updatedAt: string }> }> { return request("/api/timeline/daw-private-buses", { method: "POST", body: JSON.stringify({ action: "assign-folder", sessionId, laneIds, busId }) }); }
