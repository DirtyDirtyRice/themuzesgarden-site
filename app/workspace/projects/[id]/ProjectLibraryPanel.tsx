...
export default function ProjectLibraryPanel(props: {
  ...
  const { allTracks, linkedTrackIds, loadingLibrary, linkBusyId, linkTrack, unlinkTrack } =
    props;
      ...
      <div className="sticky top-0 z-10 bg-white space-y-2 pb-2">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            <button className={chip("All", mode === "all")} onClick={() => setMode("all")}>
              All
            </button>
            <button
        ...
        <div className="text-[11px] text-zinc-500">
          Keys: ↑ ↓ select • Enter link/unlink • Esc clear search
        </div>
      </div>
          ...
          {filtered.map((t, idx) => {
            ...
            return (
              <div
                key={tid}
                className={[
                  "rounded border p-3 flex items-center justify-between gap-3 cursor-pointer",
