import StoriesLibraryEntryCard from "./StoriesLibraryEntryCard";
import type { StoryEntry } from "./storiesTypes";

type StoriesLibrarySearchPanelProps = {
  filteredEntries: StoryEntry[];
  searchValue: string;
  folderStatus: string;
...
export default function StoriesLibrarySearchPanel({
  filteredEntries,
  searchValue,
  folderStatus,
  onSearchChange,
  ...
  onDuplicateEntry,
  onDeleteEntry,
}: StoriesLibrarySearchPanelProps) {
  return (
    <div className="rounded-2xl border border-white/15 bg-black p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-white">Search Stories</h2>
          <p className="mt-1 text-sm text-white/60">
            Search by story title, song title, tags, inspiration, notes, or
            story body.
          </p>
        </div>
