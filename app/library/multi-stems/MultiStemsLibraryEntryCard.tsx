...
type MultiStemsLibraryEntryCardProps = {
  entry: MultiStemEntry;
...
export default function MultiStemsLibraryEntryCard({
  entry,
  onEditEntry,
  onDownloadEntry,
  onDuplicateEntry,
  onDeleteEntry,
}: MultiStemsLibraryEntryCardProps) {
  return (
    <article className="rounded-xl border border-white/15 bg-black p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-2 text-xs text-white/55">
            BPM: {entry.bpm || "Unknown"} · Key: {entry.songKey || "Unknown"}
          </p>
          <p className="mt-2 text-xs text-white/55">
      ...
      <div className="mt-4 rounded-lg border border-white/10 bg-black p-3">
        ...
        <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-white/75">
          {entry.stemTypes || "No stem types listed yet."}
        </p>
      </div>
      ...
      <div className="mt-4 rounded-lg border border-white/10 bg-black p-3">
        ...
        <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-white/75">
          {entry.sourceFolder || "No source folder listed yet."}
        </p>
      </div>

      <pre className="mt-4 max-h-[240px] overflow-auto whitespace-pre-wrap rounded-lg border border-white/10 bg-black p-3 text-sm leading-6 text-white/75">
      ...
      <div className="mt-4 rounded-lg border border-white/10 bg-black p-3">
        ...
        <p className="text-xs leading-5 text-white/60">
          Project: {entry.projectLink || "None"}
        </p>
        <p className="text-xs leading-5 text-white/60">
