...
import type { CodeCapsule, CodeCapsuleState, GeneratedDeclarationKind } from "@/lib/developer-workspace/codeCapsule";
...
export default function DraftActivationWorkspace() {
  ...
  const [reservationKind, setReservationKind] = useState<GeneratedDeclarationKind>("type");
  ...
  async function reserveDeclaration() {
    if (!selected || !reservationName.trim() || !reservationReason.trim()) return;
    const confirmed = window.confirm(
      `Approve ${reservationKind} “${reservationName.trim()}” as intentionally reserved? This human exception will be timestamped and recorded with your reason.`
    );
    if (!confirmed) return;
    setBusy("reserve"); setError(""); setConfirmationToken(null);
    try {
      const updated = await capsuleRequest({
        action: "reserve-declaration",
        id: selected.id,
        expectedVersion: selected.version,
        declarationName: reservationName.trim(),
        declarationKind: reservationKind,
        reason: reservationReason.trim(),
      }) as CodeCapsule;
      setSelected(updated);
      setCapsules((current) => current.map((capsule) => capsule.id === updated.id ? updated : capsule));
      setReservationName(""); setReservationReason("");
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Declaration reservation failed."); }
    finally { setBusy(""); }
  }
  ...
  return <section className="mt-4 rounded-xl border border-violet-300/25 bg-[#0b1720] p-5">
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(300px,0.75fr)_minmax(420px,1.25fr)]">
      ...
      <div className="min-w-0 rounded-lg border border-white/10 bg-black/20 p-4">{selected ? <><div className="flex flex-wrap gap-1">{states.map((state) => <span key={state} className={`rounded border px-2 py-1 text-[10px] font-black uppercase ${state === selected.state ? "border-violet-200 bg-violet-200 text-black" : "border-white/10 text-white/30"}`}>{state}</span>)}</div><h3 className="mt-4 text-xl font-black">{selected.title}</h3><div className="mt-1 font-mono text-xs text-violet-100/60">{selected.id}</div><div className="mt-4 space-y-2">{selected.fragments.map((item) => <div key={item.id} className="rounded border border-white/10 p-3"><div className="flex justify-between gap-2 text-xs text-white/40"><span>Fragment {item.order + 1} · {item.note || "no note"}</span><span>{new Date(item.createdAt).toLocaleString()}</span></div><pre className="mt-2 overflow-auto whitespace-pre-wrap text-xs text-white/75">{item.content}</pre></div>)}</div>{["draft", "incomplete", "waiting-validation", "validated"].includes(selected.state) ? <><div className="mt-4 rounded border border-white/10 p-3"><textarea value={fragment} onChange={(event) => setFragment(event.target.value)} placeholder="Add the next inactive code fragment" className="min-h-32 w-full rounded border border-white/15 bg-black/30 px-3 py-2 font-mono text-xs" /><input value={fragmentNote} onChange={(event) => setFragmentNote(event.target.value)} placeholder="What piece does this fragment add?" className="mt-2 w-full rounded border border-white/15 bg-black/30 px-3 py-2" /><button type="button" onClick={() => void addFragment()} disabled={busy !== "" || !fragment.trim()} className="mt-2 rounded border border-violet-300/40 px-3 py-2 text-sm font-black text-violet-100 disabled:opacity-40">{busy === "fragment" ? "Storing…" : "Store inactive fragment"}</button></div><div className="mt-4 rounded border border-amber-300/25 bg-amber-300/5 p-3"><div className="font-black text-amber-100">Human-only intentional reservation</div><p className="mt-1 text-xs text-white/55">Use only when a generated declaration deliberately has no current code usage. Your reason and approval time become permanent capsule evidence.</p><div className="mt-3 grid gap-2 sm:grid-cols-[130px_1fr]"><select value={reservationKind} onChange={(event) => setReservationKind(event.target.value as GeneratedDeclarationKind)} className="rounded border border-white/15 bg-[#071016] px-2"><option value="type">type</option><option value="interface">interface</option><option value="enum">enum</option><option value="class">class</option><option value="function">function</option><option value="constant">constant</option></select><input value={reservationName} onChange={(event) => setReservationName(event.target.value)} placeholder="Exact declaration name" className="rounded border border-white/15 bg-black/30 px-3 py-2" /></div><textarea value={reservationReason} onChange={(event) => setReservationReason(event.target.value)} placeholder="Why should unused code be intentionally reserved?" className="mt-2 min-h-20 w-full rounded border border-white/15 bg-black/30 px-3 py-2" /><button type="button" onClick={() => void reserveDeclaration()} disabled={busy !== "" || !reservationName.trim() || !reservationReason.trim()} className="mt-2 rounded border border-amber-300/50 px-3 py-2 text-sm font-black text-amber-100 disabled:opacity-40">{busy === "reserve" ? "Recording human approval…" : "Approve intentionally reserved declaration"}</button></div></> : null}{(selected.intentionalReservations ?? []).length ? <details className="mt-4"><summary className="cursor-pointer text-sm font-black text-amber-100">Human-approved reservations · {(selected.intentionalReservations ?? []).length}</summary><div className="mt-2 space-y-2">{(selected.intentionalReservations ?? []).map((reservation) => <div key={`${reservation.declarationKind}:${reservation.declarationName}`} className="rounded border border-amber-300/20 px-3 py-2 text-xs"><div className="font-bold">{reservation.declarationKind} {reservation.declarationName}</div><div className="mt-1 text-white/50">{reservation.reason}</div><div className="mt-1 text-white/35">Approved by human developer · {new Date(reservation.approvedAt).toLocaleString()}</div></div>)}</div></details> : null}<div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => void validateCapsule()} disabled={busy !== "" || !selected.fragments.length || !["incomplete", "waiting-validation", "validated"].includes(selected.state)} className="rounded border border-cyan-300/50 px-4 py-2 text-sm font-black text-cyan-100 disabled:opacity-40">{busy === "validate" ? "Validating virtual project…" : "Validate assembled code"}</button>{selected.state === "validated" && confirmationToken ? <button type="button" onClick={() => void activateCapsule()} disabled={busy !== ""} className="rounded border border-emerald-300/60 bg-emerald-300/10 px-4 py-2 text-sm font-black text-emerald-100 disabled:opacity-40">{busy === "activate" ? "Activating and typechecking…" : "Confirm activation"}</button> : null}</div>{selected.validation ? <div className={`mt-4 rounded border p-3 text-sm ${selected.validation.diagnostics.length === 0 ? "border-emerald-300/30 bg-emerald-300/5" : "border-red-300/30 bg-red-300/5"}`}><div className="font-black">{selected.validation.diagnostics.length === 0 ? "Validation passed" : "Validation incomplete"}</div><div className="mt-1 text-xs text-white/50">syntax {selected.validation.syntaxPassed ? "passed" : "failed"} · requirements {selected.validation.requirementsPassed ? "passed" : "failed"} · declaration usage {selected.validation.declarationUsagePassed ? "passed" : "failed"} · project typecheck {selected.validation.projectTypecheckPassed ? "passed" : "failed"}</div>{selected.validation.diagnostics.length ? <div className="mt-2 space-y-1 font-mono text-xs text-red-100/75">{selected.validation.diagnostics.slice(0, 20).map((diagnostic) => <div key={diagnostic}>{diagnostic}</div>)}</div> : null}</div> : null}<details className="mt-4"><summary className="cursor-pointer text-sm font-black text-violet-100">Lifecycle ledger · {selected.transitions.length}</summary><div className="mt-2 space-y-2">{selected.transitions.map((transition) => <div key={transition.id} className="rounded border border-white/10 px-3 py-2 text-xs"><div className="font-bold">{transition.from ?? "created"} → {transition.to}</div><div className="mt-1 text-white/40">{new Date(transition.occurredAt).toLocaleString()} · {transition.actor} · {transition.reason}</div></div>)}</div></details></> : <div className="p-10 text-center text-sm text-white/35">Create or choose a capsule to assemble inactive code.</div>}</div>
