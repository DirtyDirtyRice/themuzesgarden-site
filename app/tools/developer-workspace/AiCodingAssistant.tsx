...
type AssistantResponse = { answer: string; model: string; citations: Array<{ path: string; startLine: number; endLine: number; reason: string }>; contextCharacters: number; context: { symbols: number; files: number; excerpts: number }; evidencePlan: { intents: string[]; evidence: string[]; focusFiles: string[]; rationale: string[] } };
...
export default function AiCodingAssistant() {
  ...
  return (
    <section className="mt-4 rounded-xl border border-emerald-300/25 bg-[#0b1720] p-5">
      ...
      <div className="mt-4 space-y-4">{exchanges.map((exchange, index) => <article key={`${exchange.question}-${index}`} className="rounded-lg border border-white/10 bg-black/20 p-4"><div className="text-xs font-black uppercase tracking-wider text-emerald-300/70">Question</div><div className="mt-1 font-bold">{exchange.question}</div><div className="mt-4 rounded-lg border border-cyan-300/20 bg-cyan-300/5 p-3"><div className="text-xs font-black uppercase tracking-wider text-cyan-200">Evidence trace</div><div className="mt-2 flex flex-wrap gap-2">{exchange.response.evidencePlan.evidence.map((engine) => <span key={engine} className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-2 py-1 text-[11px] font-bold text-cyan-100">{engine}</span>)}</div><p className="mt-2 text-xs leading-5 text-white/55">{exchange.response.evidencePlan.rationale.join(" ")}</p>{exchange.response.evidencePlan.focusFiles.length ? <div className="mt-2 text-xs text-white/45">Focused on: {exchange.response.evidencePlan.focusFiles.join(", ")}</div> : null}</div><div className="mt-4 whitespace-pre-wrap text-sm leading-6 text-white/80">{exchange.response.answer}</div><div className="mt-4 flex flex-wrap gap-2 text-xs text-white/45"><span>{exchange.response.context.symbols} symbols</span><span>·</span><span>{exchange.response.context.files} files</span><span>·</span><span>{exchange.response.context.excerpts} excerpts</span><span>·</span><span>{exchange.response.contextCharacters.toLocaleString()} characters</span></div><details className="mt-3"><summary className="cursor-pointer text-xs font-bold text-emerald-100/70">Exact source evidence</summary><div className="mt-2 space-y-1 text-xs text-white/50">{exchange.response.citations.map((citation) => <div key={`${citation.path}:${citation.startLine}`}>{citation.path}:{citation.startLine}-{citation.endLine} · {citation.reason}</div>)}</div></details></article>)}</div>
    </section>
  );
}
