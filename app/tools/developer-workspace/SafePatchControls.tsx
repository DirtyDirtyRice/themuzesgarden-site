...
export default function SafePatchControls({ proposal }: { proposal: SafePatchProposal | null }) {
  ...
  return <div className="mt-4 rounded-lg border border-cyan-300/25 bg-cyan-300/5 p-4">
    ...
    {verification ? <div className={`mt-3 rounded border p-3 text-sm ${result?.applied ? "border-emerald-300/30 bg-emerald-300/10 text-emerald-100" : "border-orange-300/30 bg-orange-300/10 text-orange-100"}`}>{result?.applied ? "Patch applied. TypeScript verification passed." : "Verification failed. The original file was restored automatically."} ({verification.diagnosticCount} diagnostics)</div> : null}
