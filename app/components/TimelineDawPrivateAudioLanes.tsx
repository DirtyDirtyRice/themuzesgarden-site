...
import TimelineDawRegionHelp from "@/app/components/TimelineDawRegionHelp";
...
export default function TimelineDawPrivateAudioLanes({ sessionId, projectId }: { sessionId: string; projectId: string }) {
  ...
  const [soloSafeIds, setSoloSafeIds] = useState(new Set<string>());
  ...
  const [activeSessionSceneProgress, setActiveSessionSceneProgress] = useState<{ currentIteration: number; totalIterations: number | null; passStartedAtMs: number; passDurationMs: number; pausedAtMs?: number }>();
  const [activeSessionScenePaused, setActiveSessionScenePaused] = useState(false);
  ...
  const [folderSendDryChecks, setFolderSendDryChecks] = useState<Record<string, Record<string, boolean>>>({});
  const [folderSendFocusChecks, setFolderSendFocusChecks] = useState<Record<string, { snapshot: Record<string, boolean>; focusedSendId: string }>>({});
  ...
  const sessionScenePauseRef = useRef<(() => void) | null>(null);
  const sessionSceneResumeRef = useRef<(() => void) | null>(null);
  ...
  const audibility = useMemo(() => {
    const result = new Map(resolveTimelineDawPrivateRoutingAudibility(lanes.map((lane) => ({ id: lane.id, busId: lane.busId, muted: lane.mix.muted, soloed: lane.mix.soloed, soloSafe: soloSafeIds.has(lane.id) })), buses.map((bus) => ({ id: bus.id, muted: bus.mix.muted, soloed: bus.mix.soloed }))));
    ...
    return result;
  }, [buses, freezes, lanes, soloSafeIds]);
  ...
  function stopTrackPreview(lane?: DawPrivateAudioLane, preserveLoopIndicator = false) {
    ...
    sessionScenePauseRef.current = null;
    sessionSceneResumeRef.current = null;
    ...
    setActiveSessionScenePaused(false);
  ...
  async function previewSessionScene(scene: TimelineDawSessionScene, followAction: TimelineDawSessionFollowAction = "stop", sceneOrderIds: string[] = [], sceneFollowActions: Record<string, TimelineDawSessionFollowAction> = {}, defaultFollowAction: TimelineDawSessionFollowAction = followAction, scenePlayCounts: Record<string, number> = {}, sceneFollowTargetIds: Record<string, string> = {}, playIteration = 1) {
    setError(undefined);
    stopTrackPreview();
    const generation = riffAuditionGenerationRef.current;
    try {
      ...
      let remainingMilliseconds = passDurationMs;
      ...
      const advanceScene = () => {
        ...
        const followIndex = followAction === "next" ? resolveTimelineDawSessionFollowTargetIndex(currentIndex, scenes, sceneFollowTargetIds[scene.id]) : createTimelineDawSessionFollowIndex(currentIndex, scenes.length, followAction);
        if (followIndex === null) {
          stopTrackPreview();
          return;
        }
      ...
      const scheduleAdvance = () => {
        startedAt = Date.now();
        previewTimerRef.current = setTimeout(advanceScene, remainingMilliseconds);
      };
      setActiveSessionSceneId(scene.id);
      setActiveSessionScenePaused(false);
      setActiveSessionSceneProgress({ currentIteration: playIteration, totalIterations: followAction === "loop" ? null : requiredPlayCount, passStartedAtMs: startedAt, passDurationMs });
      setRiffAuditionActive(true);
      setRiffAuditionProgress({ trackName: `${scene.name} scene`, trackNumber: 1, trackCount: prepared.length, passNumber: 1, passCount: 1 });
      sessionScenePauseRef.current = () => {
        if (scenePaused || !isTimelineDawRiffAuditionCurrent(generation, riffAuditionGenerationRef.current)) return;
        scenePaused = true;
        ...
        prepared.forEach(({ audio }) => audio.pause());
        setActiveSessionScenePaused(true);
        setActiveSessionSceneProgress((current) => current ? { ...current, pausedAtMs } : current);
      };
      sessionSceneResumeRef.current = () => {
        if (!scenePaused || !isTimelineDawRiffAuditionCurrent(generation, riffAuditionGenerationRef.current)) return;
        void Promise.all(prepared.map(({ audio }) => audio.play())).then(() => {
          if (!isTimelineDawRiffAuditionCurrent(generation, riffAuditionGenerationRef.current)) {
            prepared.forEach(({ audio }) => audio.pause());
            return;
          }
          scenePaused = false;
          setActiveSessionScenePaused(false);
          setActiveSessionSceneProgress((current) => current ? { ...current, passStartedAtMs: Date.now() - (current.passDurationMs - remainingMilliseconds), pausedAtMs: undefined } : current);
          scheduleAdvance();
        }).catch(() => {
          setError(`${scene.name} scene could not resume.`);
          stopTrackPreview();
        });
      };
      scheduleAdvance();
  ...
  return (
    <section className="rounded-3xl border border-violet-300/20 bg-[#080808] p-5 sm:p-7">
      ...
      <TimelineDawSessionView
        ...
        activeScenePaused={activeSessionScenePaused}
        ...
        onPauseScene={() => sessionScenePauseRef.current?.()}
        onResumeScene={() => sessionSceneResumeRef.current?.()}
      ...
      <section className="mt-4 rounded-xl border border-violet-300/25 bg-violet-300/[0.06] p-3" aria-label="Track folders">
        ...
        {Object.values(trackFolders).length ? <div className="mt-3 grid gap-2">{Object.values(trackFolders).map((folder) => {
          const sharedBusId = currentFolderBusId(folder.id);
          const sharedBus = buses.find((bus) => bus.id === sharedBusId);
          const insertCount = inserts.filter((item) => item.sourceKind === "bus" && item.sourceId === sharedBusId && !item.bypassed).length;
          const folderSends = sends.filter((item) => item.sourceKind === "bus" && item.sourceId === sharedBusId);
          const safeSendDestinationIds = sharedBus ? resolveTimelineDawTrackFolderSendDestinations(sharedBus.id, buses.map((bus) => bus.id), sends) : [];
          const dryCheckSnapshot = folderSendDryChecks[folder.id];
          const focusCheck = folderSendFocusChecks[folder.id];
          const focusShowingOriginal = Boolean(focusCheck && folderSends.every((send) => focusCheck.snapshot[send.id] === send.muted));
          const sendComparisonActive = Boolean(dryCheckSnapshot || focusCheck);
          return <div key={`${folder.id}-routing`} className="rounded-lg border border-fuchsia-300/20 bg-fuchsia-300/[0.05] p-2">
            <p className="text-xs font-black text-fuchsia-100">{folder.name} shared effects and sends</p>
            {sharedBus && focusCheck ? <button type="button" className={focusShowingOriginal ? "mt-2 rounded-xl border border-cyan-300 bg-cyan-200 px-3 py-2 text-sm font-black text-cyan-950" : `${button} mt-2`} onClick={() => { const compared = toggleTimelineDawTrackFolderSendFocusReference(sends, sharedBus.id, focusCheck.focusedSendId, focusCheck.snapshot, focusShowingOriginal); setSends(compared.sends); setMovementNotice(`${folder.name} is hearing ${compared.showingOriginal ? "the original complete send mix" : "the focused send"}.`); }}>{focusShowingOriginal ? "Hear Focused Send" : "Hear Original Mix"}</button> : null}
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <select aria-label={`${folder.name} shared effects bus`} className="min-w-48 rounded-lg border border-white/20 bg-black px-2 py-2 text-xs text-white" value={folderBusTargets[folder.id] ?? sharedBusId ?? ""} onChange={(event) => setFolderBusTargets((current) => ({ ...current, [folder.id]: event.target.value }))}><option value="">Master output (no shared bus)</option>{buses.map((bus) => <option key={bus.id} value={bus.id}>{bus.name}</option>)}</select>
              <button type="button" className={button} disabled={busy || (folderBusTargets[folder.id] ?? sharedBusId ?? "") === (sharedBusId ?? "")} onClick={() => void routeFolderToBus(folder.id, folderBusTargets[folder.id] || null)}>Route Folder to Shared Bus</button>
              {sharedBus ? <><span className="text-xs text-white/55">{sharedBus.name}: {insertCount} active inserts · {folderSends.filter((send) => !send.muted).length} active sends</span>{focusCheck && folderSends.length > 1 ? <><button type="button" className={button} disabled={focusCheck.focusedSendId === folderSends[0].id} onClick={() => { const jumped = jumpTimelineDawTrackFolderSendFocus(sends, sharedBus.id, "first"); setSends(jumped.sends); setFolderSendFocusChecks((current) => ({ ...current, [folder.id]: { ...focusCheck, focusedSendId: jumped.focusedSendId } })); setMovementNotice(`${folder.name} focus moved to the first send.`); }}>First Focus</button><button type="button" className={button} onClick={() => { const cycled = cycleTimelineDawTrackFolderSendFocus(sends, sharedBus.id, focusCheck.focusedSendId, -1); setSends(cycled.sends); setFolderSendFocusChecks((current) => ({ ...current, [folder.id]: { ...focusCheck, focusedSendId: cycled.focusedSendId } })); setMovementNotice(`${folder.name} focus moved to the previous send.`); }}>Previous Focus</button><button type="button" className={button} onClick={() => { const cycled = cycleTimelineDawTrackFolderSendFocus(sends, sharedBus.id, focusCheck.focusedSendId, 1); setSends(cycled.sends); setFolderSendFocusChecks((current) => ({ ...current, [folder.id]: { ...focusCheck, focusedSendId: cycled.focusedSendId } })); setMovementNotice(`${folder.name} focus moved to the next send.`); }}>Next Focus</button><button type="button" className={button} disabled={focusCheck.focusedSendId === folderSends[folderSends.length - 1].id} onClick={() => { const jumped = jumpTimelineDawTrackFolderSendFocus(sends, sharedBus.id, "last"); setSends(jumped.sends); setFolderSendFocusChecks((current) => ({ ...current, [folder.id]: { ...focusCheck, focusedSendId: jumped.focusedSendId } })); setMovementNotice(`${folder.name} focus moved to the last send.`); }}>Last Focus</button></> : null}{folderSends.length ? <button type="button" className={dryCheckSnapshot ? "rounded-xl border border-cyan-300 bg-cyan-200 px-3 py-2 text-sm font-black text-cyan-950" : button} disabled={Boolean(focusCheck)} onClick={() => { if (dryCheckSnapshot) { setSends((current) => restoreTimelineDawTrackFolderSendDryCheck(current, dryCheckSnapshot)); setFolderSendDryChecks((current) => { const next = { ...current }; delete next[folder.id]; return next; }); setMovementNotice(`${folder.name} shared sends restored for wet comparison.`); } else { const dry = createTimelineDawTrackFolderSendDryCheck(sends, sharedBus.id); setSends(dry.sends); setFolderSendDryChecks((current) => ({ ...current, [folder.id]: dry.snapshot })); setMovementNotice(`${folder.name} is dry for temporary A/B listening. No saved routing changed.`); } }}>{dryCheckSnapshot ? "Restore Wet Sends" : "Dry Check All Sends"}</button> : null}<select aria-label={`${folder.name} new parallel send`} className="rounded-lg border border-white/20 bg-black px-2 py-2 text-xs text-white" value={folderSendTargets[folder.id] ?? ""} disabled={!safeSendDestinationIds.length || sendComparisonActive} onChange={(event) => setFolderSendTargets((current) => ({ ...current, [folder.id]: event.target.value }))}><option value="">{safeSendDestinationIds.length ? "Choose feedback-safe destination" : "No new safe destinations"}</option>{buses.filter((bus) => safeSendDestinationIds.includes(bus.id)).map((bus) => <option key={bus.id} value={bus.id}>{bus.name}</option>)}</select>{folderSends.length ? <select aria-label={`${folder.name} new send settings`} className="rounded-lg border border-white/20 bg-black px-2 py-2 text-xs text-white" value={folderSendCopySources[folder.id] ?? ""} disabled={sendComparisonActive} onChange={(event) => setFolderSendCopySources((current) => ({ ...current, [folder.id]: event.target.value }))}><option value="">New send at 50% post-fader</option>{folderSends.map((send) => <option key={send.id} value={send.id}>Copy {buses.find((bus) => bus.id === send.destinationBusId)?.name ?? "send"} · {Math.round(send.level * 100)}% · {send.preFader ? "pre" : "post"}</option>)}</select> : null}<button type="button" className={button} disabled={!folderSendTargets[folder.id] || sendComparisonActive} onClick={() => { const destination = folderSendTargets[folder.id]; if (!destination) return; const sourceSend = folderSends.find((send) => send.id === folderSendCopySources[folder.id]); void persistSend(sourceSend ? copyTimelineDawTrackFolderSend(sourceSend, destination) : parseTimelineDawTrackFolderSend(sharedBus.id, destination)); setFolderSendTargets((current) => ({ ...current, [folder.id]: "" })); setMovementNotice(`${folder.name} now sends in parallel from ${sharedBus.name} to ${buses.find((bus) => bus.id === destination)?.name ?? "the selected bus"}${sourceSend ? " with copied mix settings" : ""}.`); }}>Add Shared Send</button></> : <span className="text-xs text-white/45">Choose an existing bus to share its Gain, Filter, Compressor, Gate, sidechain, and sends. Create or edit buses in the Bus Mixer above.</span>}
            </div>
            {folderSends.length ? <div className="mt-2 grid gap-2">{folderSends.map((send) => <div key={send.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-white/10 bg-black/35 p-2">
              <span className="min-w-36 text-xs font-black text-white">To {buses.find((bus) => bus.id === send.destinationBusId)?.name ?? "Private bus"}</span>
              <button type="button" className={focusCheck?.focusedSendId === send.id ? "rounded-xl border border-emerald-300 bg-emerald-200 px-3 py-2 text-sm font-black text-emerald-950" : button} disabled={Boolean(dryCheckSnapshot)} onClick={() => { if (focusCheck?.focusedSendId === send.id) { setSends((current) => restoreTimelineDawTrackFolderSendDryCheck(current, focusCheck.snapshot)); setFolderSendFocusChecks((current) => { const next = { ...current }; delete next[folder.id]; return next; }); setMovementNotice(`${folder.name} complete send mix restored.`); } else if (sharedBus && focusCheck) { setSends((current) => switchTimelineDawTrackFolderSendFocus(current, sharedBus.id, send.id)); setFolderSendFocusChecks((current) => ({ ...current, [folder.id]: { ...focusCheck, focusedSendId: send.id } })); setMovementNotice(`${folder.name} focus switched to ${buses.find((bus) => bus.id === send.destinationBusId)?.name ?? "the selected bus"}.`); } else if (sharedBus) { const focused = createTimelineDawTrackFolderSendFocusCheck(sends, sharedBus.id, send.id); setSends(focused.sends); setFolderSendFocusChecks((current) => ({ ...current, [folder.id]: { snapshot: focused.snapshot, focusedSendId: focused.focusedSendId } })); setMovementNotice(`${folder.name} is auditioning only the send to ${buses.find((bus) => bus.id === send.destinationBusId)?.name ?? "the selected bus"}.`); } }}>{focusCheck?.focusedSendId === send.id ? "Restore All Sends" : focusCheck ? "Switch Focus Here" : "Hear Only This Send"}</button>
              <label className="min-w-40 flex-1 text-xs text-white/60">Send level {timelineDawTrackFolderSendLevelToDb(send.level) === null ? "−∞ dB" : `${timelineDawTrackFolderSendLevelToDb(send.level)!.toFixed(2)} dB`}<input className="block w-full accent-fuchsia-300" type="range" min={0} max={2} step={0.01} value={send.level} disabled={sendComparisonActive} onChange={(event) => void persistSend(updateTimelineDawTrackFolderSend(send, { level: Number(event.target.value) }))} /></label>
              <label className="text-xs text-white/60">Exact dB<input aria-label={`${folder.name} send to ${buses.find((bus) => bus.id === send.destinationBusId)?.name ?? "bus"} decibels`} className="ml-1 w-20 rounded-lg border border-white/20 bg-black px-2 py-2 text-white" type="number" min={-60} max={6.02} step={0.1} value={(timelineDawTrackFolderSendLevelToDb(send.level) ?? -60).toFixed(2)} disabled={sendComparisonActive} onChange={(event) => void persistSend(updateTimelineDawTrackFolderSend(send, { level: timelineDawTrackFolderSendDbToLevel(Number(event.target.value)) }))} /></label>
              <button type="button" className={button} disabled={send.level === 0 || sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: nudgeTimelineDawTrackFolderSendLevelDb(send.level, -1) }))}>−1 dB</button>
              <button type="button" className={button} disabled={send.level === 0 || sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: nudgeTimelineDawTrackFolderSendLevelDb(send.level, -0.1) }))}>−0.1 dB</button>
              <button type="button" className={button} disabled={send.level === 2 || sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: nudgeTimelineDawTrackFolderSendLevelDb(send.level, 0.1) }))}>+0.1 dB</button>
              <button type="button" className={button} disabled={send.level === 2 || sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: nudgeTimelineDawTrackFolderSendLevelDb(send.level, 1) }))}>+1 dB</button>
              <button type="button" className={button} disabled={sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: timelineDawTrackFolderSendDbToLevel(-18) }))}>Preset −18 dB</button>
              <button type="button" className={button} disabled={sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: timelineDawTrackFolderSendDbToLevel(-12) }))}>Preset −12 dB</button>
              <button type="button" className={button} disabled={sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: timelineDawTrackFolderSendDbToLevel(-6) }))}>Preset −6 dB</button>
              <button type="button" className={button} disabled={send.level === 0 || sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: 0 }))}>Send Off −∞</button>
              <button type="button" className={button} disabled={send.level === 1 || sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: 1 }))}>Unity 0 dB</button>
              <button type="button" className={button} disabled={sendComparisonActive} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { preFader: !send.preFader }))}>{send.preFader ? "Pre-Fader" : "Post-Fader"}</button>
              <button type="button" className={send.muted ? "rounded-xl border border-amber-300 bg-amber-200 px-3 py-2 text-sm font-black text-amber-950 disabled:opacity-40" : button} disabled={sendComparisonActive} aria-pressed={send.muted} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { muted: !send.muted }))}>{send.muted ? "Enable Send" : "Mute Send"}</button>
              <button type="button" className={pendingFolderSendRemoval?.sendId === send.id ? "rounded-xl border border-red-300 bg-red-200 px-3 py-2 text-sm font-black text-red-950" : button} disabled={busy || sendComparisonActive} onClick={() => void removeFolderSend(send)}>{pendingFolderSendRemoval?.sendId === send.id ? "Confirm Remove Within 10s" : "Remove Send"}</button>
...
{freezes.filter((freeze) => freeze.active).map((freeze) => <audio key={freeze.id} ref={(element) => { if (element) freezeAudioRefs.current.set(freeze.id, element); else freezeAudioRefs.current.delete(freeze.id); }} src={freeze.artifact.playbackUrl} crossOrigin="anonymous" preload="metadata" />)}
      {lanes.length ? <TimelineDawRegionHelp sessionId={sessionId} /> : null}
            {lanes.length ? (
          ...
          {lanes.filter((lane) => !Object.values(trackFolders).some((folder) => folder.collapsed && folder.laneIds.includes(lane.id))).map((lane) => {
            ...
            return (
              <li key={lane.id} className="rounded-xl border bg-white/[0.04] p-3" style={{ borderColor: trackColor }}>
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-[auto_auto_auto_1fr_1fr]">
                  ...
                  <button type="button" aria-pressed={soloSafeIds.has(lane.id)} className={`${button} ${soloSafeIds.has(lane.id) ? "!bg-cyan-200" : ""}`} onClick={() => setSoloSafeIds((current) => { const next = new Set(current); if (next.has(lane.id)) next.delete(lane.id); else next.add(lane.id); return next; })}>{soloSafeIds.has(lane.id) ? "Solo Safe On" : "Solo Safe"}</button>
                  ...
                  <div className="flex flex-wrap gap-2 md:col-span-5"><button type="button" className={button} disabled={busy || lane.mix.gain === 1} onClick={() => void resetTrackMix(lane, "volume")}>Return Volume to Normal</button><button type="button" className={button} disabled={busy || lane.mix.pan === 0} onClick={() => void resetTrackMix(lane, "pan")}>Center Left / Right</button><button type="button" className={button} disabled={busy || (lane.mix.gain === 1 && lane.mix.pan === 0)} onClick={() => void resetTrackMix(lane, "both")}>Reset Volume and Center</button></div>
                </div>
