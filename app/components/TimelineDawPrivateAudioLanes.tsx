...
import { copyTimelineDawTrackFolderSend, createTimelineDawTrackFolderSendDryCheck, createTimelineDawTrackFolderSendFocusCheck, parseTimelineDawTrackFolderSend, resolveTimelineDawTrackFolderSendDestinations, resolveTimelineDawTrackFolderSendRemoval, restoreTimelineDawTrackFolderSendDryCheck, timelineDawTrackFolderSendDbToLevel, timelineDawTrackFolderSendLevelToDb, updateTimelineDawTrackFolderSend } from "@/lib/timeline/TimelineDawTrackFolderRoutingPolicy";
        ...
        {Object.values(trackFolders).length ? <div className="mt-3 grid gap-2">{Object.values(trackFolders).map((folder) => {
          ...
          return <div key={`${folder.id}-routing`} className="rounded-lg border border-fuchsia-300/20 bg-fuchsia-300/[0.05] p-2">
            ...
            {folderSends.length ? <div className="mt-2 grid gap-2">{folderSends.map((send) => <div key={send.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-white/10 bg-black/35 p-2">
              ...
              <button type="button" className={focusCheck?.focusedSendId === send.id ? "rounded-xl border border-emerald-300 bg-emerald-200 px-3 py-2 text-sm font-black text-emerald-950" : button} disabled={Boolean(dryCheckSnapshot) || Boolean(focusCheck && focusCheck.focusedSendId !== send.id)} onClick={() => { if (focusCheck?.focusedSendId === send.id) { setSends((current) => restoreTimelineDawTrackFolderSendDryCheck(current, focusCheck.snapshot)); setFolderSendFocusChecks((current) => { const next = { ...current }; delete next[folder.id]; return next; }); setMovementNotice(`${folder.name} complete send mix restored.`); } else if (sharedBus) { const focused = createTimelineDawTrackFolderSendFocusCheck(sends, sharedBus.id, send.id); setSends(focused.sends); setFolderSendFocusChecks((current) => ({ ...current, [folder.id]: { snapshot: focused.snapshot, focusedSendId: focused.focusedSendId } })); setMovementNotice(`${folder.name} is auditioning only the send to ${buses.find((bus) => bus.id === send.destinationBusId)?.name ?? "the selected bus"}.`); } }}>{focusCheck?.focusedSendId === send.id ? "Restore All Sends" : "Hear Only This Send"}</button>
