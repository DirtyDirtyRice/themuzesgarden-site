...
import { copyTimelineDawTrackFolderSend, createTimelineDawTrackFolderSendDryCheck, parseTimelineDawTrackFolderSend, resolveTimelineDawTrackFolderSendDestinations, resolveTimelineDawTrackFolderSendRemoval, restoreTimelineDawTrackFolderSendDryCheck, timelineDawTrackFolderSendDbToLevel, timelineDawTrackFolderSendLevelToDb, updateTimelineDawTrackFolderSend } from "@/lib/timeline/TimelineDawTrackFolderRoutingPolicy";
        ...
        {Object.values(trackFolders).length ? <div className="mt-3 grid gap-2">{Object.values(trackFolders).map((folder) => {
          ...
          return <div key={`${folder.id}-routing`} className="rounded-lg border border-fuchsia-300/20 bg-fuchsia-300/[0.05] p-2">
            ...
            {folderSends.length ? <div className="mt-2 grid gap-2">{folderSends.map((send) => <div key={send.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-white/10 bg-black/35 p-2">
              ...
              <label className="min-w-40 flex-1 text-xs text-white/60">Send level {timelineDawTrackFolderSendLevelToDb(send.level) === null ? "−∞ dB" : `${timelineDawTrackFolderSendLevelToDb(send.level)!.toFixed(2)} dB`}<input className="block w-full accent-fuchsia-300" type="range" min={0} max={2} step={0.01} value={send.level} disabled={Boolean(dryCheckSnapshot)} onChange={(event) => void persistSend(updateTimelineDawTrackFolderSend(send, { level: Number(event.target.value) }))} /></label>
              <label className="text-xs text-white/60">Exact dB<input aria-label={`${folder.name} send to ${buses.find((bus) => bus.id === send.destinationBusId)?.name ?? "bus"} decibels`} className="ml-1 w-20 rounded-lg border border-white/20 bg-black px-2 py-2 text-white" type="number" min={-60} max={6.02} step={0.1} value={(timelineDawTrackFolderSendLevelToDb(send.level) ?? -60).toFixed(2)} disabled={Boolean(dryCheckSnapshot)} onChange={(event) => void persistSend(updateTimelineDawTrackFolderSend(send, { level: timelineDawTrackFolderSendDbToLevel(Number(event.target.value)) }))} /></label>
              <button type="button" className={button} disabled={send.level === 0 || Boolean(dryCheckSnapshot)} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: 0 }))}>Send Off −∞</button>
              <button type="button" className={button} disabled={send.level === 1 || Boolean(dryCheckSnapshot)} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { level: 1 }))}>Unity 0 dB</button>
              <button type="button" className={button} disabled={Boolean(dryCheckSnapshot)} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { preFader: !send.preFader }))}>{send.preFader ? "Pre-Fader" : "Post-Fader"}</button>
              <button type="button" className={send.muted ? "rounded-xl border border-amber-300 bg-amber-200 px-3 py-2 text-sm font-black text-amber-950 disabled:opacity-40" : button} disabled={Boolean(dryCheckSnapshot)} aria-pressed={send.muted} onClick={() => void persistSend(updateTimelineDawTrackFolderSend(send, { muted: !send.muted }))}>{send.muted ? "Enable Send" : "Mute Send"}</button>
              <button type="button" className={pendingFolderSendRemoval?.sendId === send.id ? "rounded-xl border border-red-300 bg-red-200 px-3 py-2 text-sm font-black text-red-950" : button} disabled={busy || Boolean(dryCheckSnapshot)} onClick={() => void removeFolderSend(send)}>{pendingFolderSendRemoval?.sendId === send.id ? "Confirm Remove Within 10s" : "Remove Send"}</button>
