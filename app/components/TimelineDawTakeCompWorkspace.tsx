...
export default function TimelineDawTakeCompWorkspace({ sessionId, takes }: { sessionId: string; takes: DawRecordingTake[] }) {
  ...
  async function promoteComp(comp: DawTakeComp) {
    setBusy(true);
    setError(undefined);
    try {
      const result = await promoteDawTakeComp(sessionId, comp.id);
  ...
  return (
    <div className="mt-6 rounded-2xl border border-cyan-300/20 bg-cyan-300/[0.04] p-4">
      ...
      {comps.length ? <div className="mt-3 grid gap-2">{comps.map((comp) => <div key={comp.id} className="rounded-xl border border-white/15 bg-black p-3"><div className="flex flex-wrap items-center gap-2"><button type="button" className="text-sm font-black text-white underline decoration-white/30" onClick={() => editComp(comp)}>{comp.name}</button>{comp.render ? <span className="text-xs font-black text-emerald-200">Rendered {comp.render.durationSeconds.toFixed(2)}s · {(comp.render.byteLength / 1_048_576).toFixed(2)} MB</span> : <span className="text-xs text-white/45">Recipe only</span>}{comp.promotion ? <span className={`text-xs font-black ${comp.promotion.current ? "text-violet-200" : "text-amber-200"}`}>{comp.promotion.current ? "Promoted source current" : "Promoted source superseded"}</span> : null}{renderProgress[comp.id] !== undefined ? <span className="text-xs font-black text-cyan-200">Render {renderProgress[comp.id]}%</span> : null}</div><div className="mt-2 flex flex-wrap gap-2"><button type="button" className={control} disabled={busy} onClick={() => void renderComp(comp)}>{renderingCompId === comp.id ? "Rendering…" : comp.render ? "Render Again" : "Render WAV"}</button>{comp.render ? <button type="button" className={control} disabled={busy} onClick={() => void loadDelivery(comp)}>{deliveryUrls[comp.id] ? "Refresh Delivery" : "Audition & Download"}</button> : null}{comp.render ? <button type="button" className={control} disabled={busy || Boolean(comp.promotion?.current)} onClick={() => void promoteComp(comp)}>{comp.promotion?.current ? "Promoted to Session" : comp.promotion ? "Re-promote New Render" : "Promote to Session"}</button> : null}<button type="button" className={control} disabled={busy} onClick={() => void removeComp(comp)}>Delete Recipe</button></div>{deliveryUrls[comp.id] ? <><audio className="mt-2 w-full" controls preload="metadata" src={deliveryUrls[comp.id]} /><a className="mt-2 inline-block text-sm font-black text-cyan-200" href={deliveryUrls[comp.id]} download={`${comp.name}.wav`}>Download rendered WAV</a></> : null}</div>)}</div> : null}
