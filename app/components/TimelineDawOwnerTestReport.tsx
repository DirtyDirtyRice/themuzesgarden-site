...
type Data = {
  ...
  report: { generatedAt: string; privacy: string; findings: Finding[]; verifiedCount: number; humanRequiredCount: number; attentionRequiredCount: number; screenshotCount: number; acceptance: { status: "passed" | "held"; passed: boolean; verifiedCount: number; requiredCount: number; blockers: string[] } };
};
...
export default function TimelineDawOwnerTestReport({ sessionId }: { sessionId: string }) {
  ...
  return (    <section id="owner-test-report" className="rounded-3xl border-2 border-cyan-300/40 bg-cyan-300/[.05] p-5 print:fixed print:inset-0 print:z-[9999] print:overflow-visible print:bg-white print:text-black">
      ...
      {open && data ? <div className="mt-5">
        <div className={`rounded-xl border p-4 ${data.report.acceptance.passed ? "border-emerald-300/40 bg-emerald-300/10" : "border-amber-300/40 bg-amber-300/10"}`} role="status">
          <p className="text-xs font-black uppercase tracking-[.18em]">Real-musician end-to-end acceptance</p>
          <h3 className="mt-1 text-2xl font-black">{data.report.acceptance.passed ? "ACCEPTANCE PASSED" : "ACCEPTANCE HELD"}</h3>
          <p className="mt-1">{data.report.acceptance.verifiedCount}/{data.report.acceptance.requiredCount} musician paths verified by technical evidence and human judgment.</p>
          {data.report.acceptance.blockers.length ? <ul className="mt-2 list-disc pl-5 text-sm">{data.report.acceptance.blockers.map((blocker) => <li key={blocker}>{blocker}</li>)}</ul> : <p className="mt-2 text-sm font-bold">All protected-source, import, audition, edit, mix, recovery, and export checks passed.</p>}
        </div>
