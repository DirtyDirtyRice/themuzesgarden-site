...
export default function TimelineDawMusicianRiffMatch({ lanes, selectedIds, waveforms }: {
  ...
  return <section className="mt-3 rounded-xl border border-cyan-300/25 bg-cyan-300/5 p-3">
    <div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-black text-cyan-100">Find Matching Riffs in Selected Tracks</h3><p className="text-xs text-white/55">Compares real waveform and attack shapes. Only regions matching 90% or better across every selected track are colored. This first pass compares sound patterns; exact note transcription comes later.</p></div><button type="button" className="rounded-xl border border-white/25 bg-white px-3 py-2 text-sm font-black text-black disabled:opacity-40" disabled={!ready} onClick={analyze}>Analyze Selected Tracks</button></div>
    ...
    {matches !== null ? <div className="mt-3">{matches.length ? <><p className="text-sm font-black">Found {matches.length} matching riff {matches.length === 1 ? "family" : "families"}.</p><div className="mt-2 grid gap-3">{selected.map((lane) => <div key={lane.id}><p className="text-xs font-black">{lane.name}</p><div className="relative mt-1 h-8 overflow-hidden rounded-lg border border-white/15 bg-black/50">{matches.flatMap((match) => match.regions.filter((region) => region.laneId === lane.id).map((region) => <span key={`${match.id}:${lane.id}`} className="absolute inset-y-0 border-x border-white/60" title={`${match.id}: ${Math.round(region.similarity * 100)}% match, ${region.startSeconds.toFixed(1)}–${region.endSeconds.toFixed(1)} seconds`} style={{ backgroundColor: match.color, left: `${100 * region.startSeconds / Math.max(0.001, lane.sourceOutSeconds - lane.sourceInSeconds)}%`, width: `${100 * (region.endSeconds - region.startSeconds) / Math.max(0.001, lane.sourceOutSeconds - lane.sourceInSeconds)}%` }} />))}</div></div>)}</div><ul className="mt-3 flex flex-wrap gap-2">{matches.map((match) => <li key={match.id} className="rounded-lg border border-white/15 px-2 py-1 text-xs font-black" style={{ borderColor: match.color }}>{match.id.replace("riff-", "Riff family ")} · {Math.round(match.similarity * 100)}%</li>)}</ul></> : <p className="text-sm text-amber-100">No regions reached the 90% match requirement across all selected tracks.</p>}</div> : null}
  </section>;
}
