...
export default function TimelineDawMusicianRiffMatch({ lanes, selectedIds, waveforms, onAudition, onAuditionFamily, auditionActive, auditionProgress, onStopAudition }: {
  ...
  return <section className="mt-3 rounded-xl border border-cyan-300/25 bg-cyan-300/5 p-3">
    <div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-black text-cyan-100">Find Matching Riffs in Selected Tracks</h3><p className="text-xs text-white/55">Compares real waveform and attack shapes. Only regions matching 90% or better across every selected track are colored. This first pass compares sound patterns; exact note transcription comes later.</p></div><div className="flex flex-wrap gap-2"><button type="button" className="rounded-xl border border-white/25 bg-white px-3 py-2 text-sm font-black text-black disabled:opacity-40" disabled={!ready} onClick={analyze}>Analyze Selected Tracks</button>{auditionActive ? <button type="button" className="rounded-xl border border-red-600 bg-red-100 px-3 py-2 text-sm font-black text-red-950" onClick={onStopAudition}>Stop Riff Comparison</button> : null}</div></div>
