...
export default function TimelineDawMusicianTrialWorkspace({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => { let active = true; void loadTimelineDawMusicianTrialTake(sessionId).then((take) => { if (active && take) { show(take.wav); setMessage("Your saved browser-private test take was reopened."); } }).catch(() => setError("A previous browser-private test take could not be reopened.")); return () => { active = false; }; }, [sessionId]);
  ...
  async function stop() {
    ...
    try {
      ...
      await saveTimelineDawMusicianTrialTake({ sessionId, wav: blob, savedAt: new Date().toISOString() }); setMessage("Test take saved privately in this browser. Refresh the page to confirm it reopens.");
  ...
  async function exportEdit() {
    ...
    try {
      ...
      anchor.href = download; anchor.download = `musician-test-${sessionId}.wav`; anchor.click(); setTimeout(() => URL.revokeObjectURL(download), 1_000); setMessage("Edited WAV exported. Tell the owner whether this process felt clear.");
