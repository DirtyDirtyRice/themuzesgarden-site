...
function getTrackSectionById(
  ...
  const found = sections.find((section) => String(section?.id ?? "") === needle);
...
export function useAudioEngine(args: {
    ...
    (target: PlaybackTarget) => {
      ...
      const resolvedStartTime =
        typeof target.startTime === "number" && Number.isFinite(target.startTime)
          ...
          : resolvedSection
          ? clampNonNegative(resolvedSection.start)
          : 0;
      ...
      const labelSuffix = resolvedSection
        ? ` • ${resolvedSection.description?.trim() || resolvedSection.id}`
        : resolvedStartTime > 0
        ? ` • @ ${fmtTime(resolvedStartTime)}`
        : "";

      setNowLabel(`${labelBase}${labelSuffix}`);

      currentSectionIdRef.current = resolvedSectionId;
      currentSectionStartRef.current = resolvedStartTime > 0 ? resolvedStartTime : null;
      ...
      writePersisted({
        ...
        sectionStartTime: resolvedStartTime > 0 ? resolvedStartTime : null,
      });
      ...
      logProjectPlayIfPossible(track, {
        ...
        startTime: resolvedStartTime > 0 ? resolvedStartTime : undefined,
      });
  ...
  const resumeLastSession = useCallback(() => {
    ...
    const sectionId = typeof p.currentSectionId === "string" ? p.currentSectionId : null;
  ...
  useEffect(() => {
    ...
    const t = allTracks.find((x) => String(x.id) === String(nid));
    if (!t) return;

    const activeSection = getTrackSectionById(t, currentSectionIdRef.current);
    const labelBase = `${t.title ?? "Untitled"} — ${t.artist ?? "Supabase"}`;
    const labelSuffix = activeSection
      ? ` • ${activeSection.description?.trim() || activeSection.id}`
      : "";
