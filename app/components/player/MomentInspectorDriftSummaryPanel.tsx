...
export default function MomentInspectorDriftSummaryPanel(props: {
  rows: InspectorPhraseDriftFamilyRow[];
}) {
  const { rows } = props;
        ...
        {visibleRows.map((row) => (
          ...
          >
            ...
            <div className="shrink-0 text-zinc-700">
              {getPhraseDriftLabelUi(row.dominantDriftLabel)}
              {" • "}
              {getPhraseDriftSeverityUi(row.highestSeverity)}
              {` • unstable ${row.unstableCount} • members ${row.comparedMemberCount} • health ${formatWholePercent(
                row.driftHealthScore
              )}`}
            </div>
          </div>
        ))}
