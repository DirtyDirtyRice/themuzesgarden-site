...
const capabilities = [
  ...
  ["Code holding", "Keeps incomplete generated code in inactive capsules. Every new type, interface, enum, class, function, or constant needs verified usage or a timestamped human-approved reservation before activation."],
...
const validations = [
  ...
  ["Safety gates", "Held-code activation, generated-declaration usage, human approval, and architectural regression gates reject incomplete or unsafe changes."],
...
export default function DeveloperWorkspaceDocumentationPage() {
  return (
    <main className="mx-auto max-w-7xl p-4 pb-24">
      ...
      <section className="mt-5 rounded-xl border border-amber-300/25 bg-[#0b1720] p-5"><h2 className="text-2xl font-black">What has already been validated</h2><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">{validations.map(([title, detail]) => <article key={title} className="rounded-lg border border-white/10 bg-black/20 p-4"><h3 className="font-black text-amber-100">{title}</h3><p className="mt-2 text-sm leading-6 text-white/55">{detail}</p></article>)}</div><p className="mt-4 text-sm text-white/50">The automated beta baseline currently contains 43 focused checks across 12 test files. A passing automated suite does not replace the two human scenarios above.</p></section>

      <section className="mt-5 rounded-xl border border-amber-300/25 bg-[#0b1720] p-5"><h2 className="text-2xl font-black">Realized declaration rule</h2><p className="mt-2 text-sm leading-6 text-white/60">AI-generated declarations do not become active merely because they compile. Validation traces each newly generated type, interface, enum, class, function, and constant to a real code usage. An unused declaration remains unrealized inside its capsule unless a human deliberately approves it as intentionally reserved.</p><ol className="mt-3 space-y-2 text-sm leading-6 text-white/60"><li><span className="font-black text-amber-100">1.</span> Validate the capsule to identify unused declarations.</li><li><span className="font-black text-amber-100">2.</span> Prefer adding a real, verified usage relationship.</li><li><span className="font-black text-amber-100">3.</span> If future use is intentional, enter the exact declaration name and a concrete reason, then confirm the human-only reservation prompt.</li><li><span className="font-black text-amber-100">4.</span> Revalidate. The approval identity, reason, and timestamp remain in the lifecycle evidence.</li><li><span className="font-black text-amber-100">5.</span> Separately confirm activation. Reservation approval never activates code by itself.</li></ol></section>

      <section className="mt-5 grid gap-5 lg:grid-cols-2"><article className="rounded-xl border border-white/10 bg-[#0b1720] p-5"><h2 className="text-xl font-black">Installation and requirements</h2><ul className="mt-3 space-y-2 text-sm leading-6 text-white/60"><li>Run on the coder’s own computer through the local development server.</li><li>Use a current Node.js runtime and install the project dependencies.</li><li>Open <code>/developer-workspace</code> in Chrome and use Install app when Chrome offers it.</li><li>Set the server-only AI key only when AI assistance is required; never place it in browser storage.</li><li>Keep the project under normal source control. The workspace is an additional safety layer, not a replacement for Git.</li></ul></article><article className="rounded-xl border border-red-300/20 bg-[#0b1720] p-5"><h2 className="text-xl font-black">Beta safety boundaries</h2><ul className="mt-3 space-y-2 text-sm leading-6 text-white/60"><li>Use only projects the tester is authorized to access.</li><li>Review every proposed edit before activation.</li><li>Do not treat AI predictions as proof; TypeScript and production verification remain authoritative.</li><li>AI cannot grant an intentionally-reserved exception; a human must name it, explain it, and confirm it.</li><li>Report unexpected file changes immediately and preserve the project state.</li><li>Do not send proprietary source code in feedback. Use the privacy-safe support report.</li></ul></article></section>
    </main>
  );
}
