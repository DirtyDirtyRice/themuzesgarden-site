...
export class TimelineDawPrivateFreezeRenderer {
  render(lanes: TimelineDawPrivateFreezeLaneInput[], outputInserts: TimelineDawPrivateInsert[] = [], outputAutomation: TimelineDawPrivateAutomationEnvelope[] = [], master: TimelineDawPrivateMaster = {gain:1,muted:false,revision:0}): { channels: Float32Array[]; sampleRate: number; frameCount: number; meter: ReturnType<typeof measureTimelineDawPrivateMaster>; dynamicsMeters: TimelineDawPrivateDynamicsMeter[] } {
    ...
    for (const lane of lanes) {
      ...
      let transformed=lane.transform?transformTimelineDawPrivateLanePcm(local,lane.transform,lane.protectedFrames??[],sampleRate,lane.warpMarkers??[]):local; const detectorLane=lanes.find(x=>lane.inserts.some(i=>i.sidechain?.sourceKind==="lane"&&i.sidechain.sourceId===x.id)),detector=detectorLane?[detectorLane.audio.channels[0],detectorLane.audio.channels[Math.min(1,detectorLane.audio.channelCount-1)]]:undefined;dynamicsMeters.push(...process(transformed,sampleRate,lane.inserts,detector));const latency=lane.inserts.reduce((n,x)=>n+timelineDawPrivateInsertLatency(x),0);transformed=applyTimelineDawPrivateDelay(transformed,maximumLatency-latency);
