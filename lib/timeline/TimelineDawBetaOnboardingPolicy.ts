...
export type TimelineDawBetaReleaseInput = { enrolled: boolean; acknowledged: boolean; environment: TimelineDawBetaEnvironment; workflowPercent: number; workflowComplete: boolean; exportReady: boolean; isolatedTrialReady?: boolean; blockingFeedback: number; unresolvedFeedback: number; integrityBlockers: number };
...
export function evaluateTimelineDawBetaRelease(input: TimelineDawBetaReleaseInput) {
  ...
  const blockers = [
    ...
    ...(!input.isolatedTrialReady && (input.workflowPercent < 100 || !input.workflowComplete) ? ["Guided workflow is incomplete."] : []),
    ...(!input.isolatedTrialReady && !input.exportReady ? ["A verified export is not ready."] : []),
  ...
  const warnings = [
    ...(input.unresolvedFeedback > input.blockingFeedback ? [`${input.unresolvedFeedback - input.blockingFeedback} non-blocking report(s) remain open.`] : []),
    ...(input.isolatedTrialReady && (input.workflowPercent < 100 || !input.workflowComplete || !input.exportReady) ? ["The older owner workflow record is incomplete; it does not block the isolated seven-action musician trial."] : []),
  ];
  return { ready: blockers.length === 0, blockers, warnings, environmentFailures };
}
